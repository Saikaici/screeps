var roleRemoteBuilder = {
    ...
    run: function(creep) {
		...
		//
			...
			// This is handled by the remoteClaimer role
			/*
			if(creep.signController(creep.room.controller, 'RIP suborbitalbacon') == ERR_NOT_IN_RANGE) {
				creep.moveTo(creep.room.controller);
			}
			//When this is done, set claimed flag for the room to true so this portion doesn't execute anymore
			if(creep.room.controller.sign.text == 'RIP suborbitalbacon')
			{
		...
		//Now for the fun part of getting minerals from a node, 
			...
			var closestConstructionSite = creep.pos.findClosestByPath(FIND_CONSTRUCTION_SITES, {filter: (ConstructionSite) => { return (ConstructionSite.structureType == STRUCTURE_ROAD) || (ConstructionSite.structureType == STRUCTURE_WALL)  || (ConstructionSite.structureType == STRUCTURE_TERMINAL) || (ConstructionSite.structureType == STRUCTURE_TOWER) || (ConstructionSite.structureType == STRUCTURE_STORAGE) || (ConstructionSite.structureType == STRUCTURE_CONTAINER) || (ConstructionSite.structureType == STRUCTURE_EXTENSION) || (ConstructionSite.structureType == STRUCTURE_SPAWN) || (ConstructionSite.structureType == STRUCTURE_LINK) || (ConstructionSite.structureType == STRUCTURE_RAMPART) }}, closestConstructionSitePos);
		...
		// Part C
        //-----------------------------------------------------------------------------------------------------------------------------------------------------------
		// If not building, it needs to harvest from the nearest node, or grab stuff from containers when they get built.
