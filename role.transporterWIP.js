var roleTransporter= {
    ...
    run: function(creep, removalContainers, depositContainersEnter) {
        ...
        else if(((creep.room.energyAvailable)/(creep.room.store.energyCapacityAvailable) < 1) && (creep.alphaTransporter)) {
            //If it's the alpha transporter, it can withdraw from whatever it needs to fill the extensions for further spawning.
            var maxEnergyCheck = creep.store.getMaxCapacity(RESOURCE_ENERGY);
            var target =  creep.findClosestByPath(FIND_STRUCTURES, {
                filter: (structure) => {
                    return (((structure.structureType == STRUCTURE_STORAGE) || (structure.structureType == STRUCTURE_CONTAINER)) && (structure.store.getFreeCapacity(RESOURCE_ENERGY) > maxEnergyCheck));
                }
            });
            console.log(target);
            if(target != null) {
